...
printf("instr %d\n", cnt++);
  switch (i->kind) {
  case I_OPER:
    fprintf(out, "OPER: %s", i->u.OPER.assem);
    break;
  case I_LABEL:
    fprintf(out, "LABEL: %s", i->u.LABEL.assem);
    break;
  case I_MOVE:
    fprintf(out, "MOVE: %s", i->u.MOVE.assem);
    break;
  }
return;
*/
  ...
  switch (i->kind) {
  case I_OPER:
    AS_format(r, i->u.OPER.assem, i->u.OPER.dst, i->u.OPER.src, i->u.OPER.jumps, m);
    fprintf(out, "%s", r);
    break;
  case I_LABEL:
    AS_format(r, i->u.LABEL.assem, NULL, NULL, NULL, m); 
    fprintf(out, "%s", r); 
    /* i->u.LABEL->label); */
    break;
  case I_MOVE:
    AS_format(r, i->u.MOVE.assem, i->u.MOVE.dst, i->u.MOVE.src, NULL, m);
    fprintf(out, "%s", r);
    break;
  }
}
